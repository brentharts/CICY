...
### A Python implementation of the finite layer of the construction, with every number reached twice where it can be, thirty facts machine-checked, and a ledger of what is quoted
...
5. **Thirty facts machine-checked**, Mathlib-free, several universally
...
The work was done in five parts, each a code, a test and a documentation
commit, and this write-up is the sixth. `status()`, `status_periods()`,
...
kernel-checked by Lean 4.33.0 in about three seconds. The concrete ones cover
...
permits. 25 of the 30 facts are also choice-free; the five that are not are the
...
python3 tests/test_sixsphere.py          # 246 checks without Lean, 254 with
...
- **Other triangle groups.** Nothing in the lattice layer is special to
  (3, 4, ∞) except the matrices. Which other representations `ρ: Δ → SL(Z⁴)`
  with a unipotent cusp admit twists giving `|p| = 1`, and whether any of them
  pass the paper's analytic conditions, is a finite search on the lattice side
  and an open question on the other.
